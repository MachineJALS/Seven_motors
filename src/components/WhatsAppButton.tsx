import { armarLinkWhatsApp } from "../config";
...
/**
 * Botón que abre WhatsApp con un mensaje pre-cargado.
 * Usa el link gratuito wa.me — no la API de pago de WhatsApp Business.
 */
export default function WhatsAppButton({ mensaje, vendido, className }: Props) {
  if (vendido) {
    return (
      <span className={`btn-whatsapp btn-whatsapp--vendido ${className ?? ""}`}>
        Vehículo vendido
      </span>
    );
  }
  ...
  return (
    <a
      ...
      href={armarLinkWhatsApp(mensaje)}
      target="_blank"
